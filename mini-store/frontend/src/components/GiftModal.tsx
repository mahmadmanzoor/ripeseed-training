...
interface GiftModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (product: Product, quantity: number, recipientEmail: string, message?: string) => Promise<void>;
...
const GiftModal = ({ 
  ...
  const handleConfirm = async () => {
    if (canAfford && !isLoading && recipientEmail.trim()) {
      await onConfirm(product, quantity, recipientEmail.trim(), message.trim() || undefined);
      setQuantity(1);
      setRecipientEmail('');
      setMessage('');
    }
  ...
  const isFormValid = recipientEmail.trim() && isValidEmail(recipientEmail.trim()) && canAfford && !isLoading;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">🎁 Send as Gift</h2>
          <button
            onClick={handleClose}
            disabled={isLoading}
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 ${
                recipientEmail && !isValidEmail(recipientEmail) ? 'border-red-300' : 'border-gray-300'
              }`}
            />
          ...
          <div className="bg-purple-900 rounded-lg p-4 mb-6">
            <div className="flex justify-between items-center">
              <span className="text-white font-medium">Gift Total:</span>
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-red-600 text-sm">
                    You need ${(totalAmount - currentWalletBalance).toFixed(2)} more to send this gift.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex space-x-3 p-6 border-t border-gray-200">
          ...
          >
            {isLoading ? (
              ...
              `🎁 Send Gift for $${totalAmount.toFixed(2)}`
            )}
          </button>
        </div>
      </div>
